...
function newIntercity(type) {
    ...
    // 开始便利id("")id("")id("startTimeAndNum")
    do {
        ordersList = id("rvContent").findOnce();
        refview = id("iv_bottom_update").findOnce();
        if (!ordersList) {
            console.log("[!] 找不到订单列表");
            errorCount++;
        }
        ...
        ordersList.children().forEach(function(child) {
            if (!child || allcheck_stat) return; //className("android.view.ViewGroup")
            if (child.className() != 'android.view.ViewGroup') return;
            try {
                let dateview = child.findOne(id("startTimeAndNum"))|| {text: ()=> {return 'null'}};
                let distance = child.findOne(id("tvStartInnerDistance")) || {text: ()=> {return 'null'}};
                let people = child.findOne(id("tvRemark")) || {text: ()=> {return 'null'}};
